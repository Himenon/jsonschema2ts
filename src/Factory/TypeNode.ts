...
export const create = ({ factory }: Context): Factory["create"] => (params: Params$Create): ts.TypeNode => {
  const literalTypeNode = LiteralTypeNode.create({ factory });
  const unionTypeNode = UnionTypeNode.create({ factory });
  const createNode = () => {
    switch (params.type) {
      case "string":
        if (params.enum) {
